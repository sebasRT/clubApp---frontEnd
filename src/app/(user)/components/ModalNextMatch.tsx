...
const ModalNextMatch = ({data, match}: any) => {
  ...
  return (
    <div className={`fixed z-10 inset-0 max-w-screen max-h-[32rem] flex justify-center ${closeModal ? 'hidden' : ''} `} >
      <div className="fixed inset-0 w-screen h-screen bg-[#d9d9d9b4]"  onClick={changeModal}/>
        <div className="relative flex flex-col bg-silver-50 bg-opacity-90 rounded-[5px] text-center gap-7  my-auto py-4 mx-2 text-wrap w-[25rem] items-center">
          <button className=" absolute top-5 right-5 cursor-pointer " onClick={changeModal}>x</button>
